...
function DataLogs(){
    ...
    const [api,setApi] = useState(URL+`/datalogs?searchKey=${params.searchKey}&searchValue=${params.searchValue}&page=${params.page}&limit=${params.limit}`);

    // console.log(api);

// http://localhost:3001/data2
// http://localhost:3001/function?searchKey=light&searchValue=400
// http://localhost:3001/data2?page=&limit=
    const fetchApi = (link) =>{
        fetch(link)
        .then(res => res.json())
        .then(data => {
...
// mình đang muốn là 1 đường dẫn sử dụng chung cho toàn bộ các yêu cầu
    ...
    useEffect(()=>{
        // console.log("2");
        setApi(URL+`/datalogs?searchKey=${params.searchKey}&searchValue=${params.searchValue}&page=${params.page}&limit=${params.limit}&sortKey=${params.sortKey}&sortValue=${params.sortValue}`);
    },[params]);
                ...
                <div className="option">
                    ...
                    <div className="select">
                        <select value ={selectKey} className="select__list" onChange={handleChangeSelectKey} >
                            ...
                            <option className="select__option5" value={'windspeed'}>windspeed</option>
